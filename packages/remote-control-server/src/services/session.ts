...
function toResponse(row: { id: string; environmentId: string | null; title: string | null; status: string; source: string; permissionMode: string | null; workerEpoch: number; createdAt: Date; updatedAt: Date }): SessionResponse {
  return {
    id: row.id,
...
export function createSession(req: CreateSessionRequest): SessionResponse {
...
function toSummaryResponse(row: { id: string; title: string | null; status: string; updatedAt: Date }): SessionSummaryResponse {
  return {
    id: row.id,
    title: row.title,
    status: row.status,
